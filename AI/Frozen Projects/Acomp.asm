...
;REV 6 LAST UPDATED 19/6/2009  1455

;CODE COMPLETED 21/06/2009 2205 ::: WORKING TESTED ::: PERFECT :: GLITCH00:DURING POWERDOWN SENT '0' NOT RECEIVED
...
START:		NOP
			MOV SP,#17H					;RELOCATE STACK OVER 17H
...
STATREQ:	JNB 7AH,NOREQ
			CLR 7AH
			SETB 7BH					;USED BY SERIAL ISR
			SETB PSW.4					;SWITCH TO BANK 3
			MOV R1,#95					;SEND 96 CHARACTER STATUS USING TI INTERRUPT :: CHECK SERIAL ISR FOR SENDING
			MOV R0,#80H
			MOV SBUF,@R0
			INC R0
			CLR	PSW.4
...
BROWNOUT:	JB 7CH,PSAFEB	
			MOV A,5FH					;BATTERY A TESTED FOR > 11.53V
			CJNE A,#196,CHPWR0
CHPWR0:		JNC PSAFEB
			SETB 7CH					;BIT 7CH INDICATES SERIAL REPORTING DONE
			MOV SBUF,#'A'				;SEND OUT 'A' TO INDICATE POWER PROBLEM
PSAFEB:		JB 7DH,PSAFE
			MOV A,62H					;BATTERY B TESTED FOR > 11.53V
			CJNE A,#196,CHPWR1
CHPWR1:		JNC PSAFE
			JNB 7CH,NOPROB
			LCALL SERWAIT
NOPROB:		SETB 7DH					;BIT 7DH INDICATES SERIAL REPORTING DONE
			MOV SBUF,#'B'			  	;SEND OUT 'B' TO INDICATE POWER PROBLEM
PSAFE:		NOP


BROUT11:	JNB 7CH,SAFE1				;IF ANY BATTERY DROPS BELOW 11.00V THEN FLOOD SERIAL CHANNEL WITH 'B'
			SJMP B1100
SAFE1:		JNB 7DH,SAFEALL
B1100:		MOV A,5FH
			CJNE A,#187,CPWR0
CPWR0:		JNC CHKB
			SJMP TXDBB
...
SERCON:		CLR TR2
			...
			MOV A,SBUF
			JB 7FH,SKIPC
			CJNE A,#34H,SKIPC3			;BIT 7FH SET IMPLIES CONNECTION ESTABLISHED
...
SKIPC:		CJNE A,#35H,SKIPC2		   	;SEND DISCONNECT AND DROP CONNECTION IF DISCONNECT IS RECEIVED
			...
			CLR 7FH
			SJMP EXITC
SKIPC2:		CJNE A,#'S',SKIPC3
			SETB 7AH	 				;7AH INDICATES MASTER REQUESTING FOR STATUS
			SJMP EXITC
SKIPC3:		CJNE A,#'Z',EXITC
			MOV SBUF,#30H
			SETB 79H					;SET 79H TO INDICATE POWER DOWN IN CASE OF BROWN OUT
			SJMP EXITC								
TXI:		CLR TI
			JNB 7BH,EXITC				;SEND STATUS REQUEST FROM THE MASTER
			SETB PSW.4
			MOV SBUF,@R0
			INC R0
			DJNZ R1,NIOMI
			CLR 7BH						;REQUEST COMPLETED
NIOMI:		CLR PSW.4
EXITC:		POP ACC
			SETB TR2
			NOP
